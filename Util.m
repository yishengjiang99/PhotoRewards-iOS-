...
void httpPostPrompt(NSString* post, NSString* url, HttpRetBlock completion){
    NSString *t=[NSString stringWithFormat:@"%.0f",[[NSDate date] timeIntervalSince1970]];
    NSString *h=[Util convertIntoMD5:[NSString stringWithFormat:@"%@%@what1sdns?",t,getIdfa()]];
    post=[NSString stringWithFormat:@"%@&idfa=%@&mac=%@&cb=%@&t=%@&h=%@&uid=%d",post,getIdfa(),getMacAddress(),getAppName(),t,h,getUid()];
    NSLog(@"posting %@ to url %@",post,url);
...
{
	NSMutableDictionary *infoo=[NSMutableDictionary dictionary];
	for(NSString *k in dictionary){
		id v =[dictionary objectForKey:k];
		if([v isMemberOfClass:[NSDictionary class]] || [v isMemberOfClass:[NSArray class]] ){
			[infoo setObject:json_encode(v)	forKey:k];
		}else if([v isMemberOfClass:[NSData class]]){
			[infoo setObject:[[NSString alloc] initWithData:v encoding:NSUTF8StringEncoding] forKey:k];
		}
		else{
			[infoo setObject:v forKey:k];
		}
	}
	
	
	NSData* nsdata = [NSJSONSerialization dataWithJSONObject:infoo options:kNilOptions error:nil];
...
+(NSDictionary *) getConfigs
{
    ...
    NSDictionary* config=
    [Util getJson: [NSString stringWithFormat:@"http://www.json999.com/checkin.php?idfa=%@&cb=%@&idfa=%@",
                    idfa,callback,getIdfa()]];
